...
from .layers import PatchEmbed, Block, BlockBA
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features(self, x, masks=None):
        if isinstance(x, list):  # actually, no use
...
##############################################################################
def vit_small_ba(patch_size=16, **kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
        num_heads=6,
        mlp_ratio=4,
        block_fn=partial(BlockBA, attn_class=_get_attn_class(**kwargs)),
        **kwargs,
    )
    return model


def vit_base_ba(patch_size=16, **kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
...
def vit_large_ba(patch_size=16, **kwargs):
    model = DinoVisionTransformer(
        patch_size=patch_size,
        embed_dim=1024,
        depth=24,
...
def vit_giant2_ba(patch_size=16, **kwargs):
    """
    Close to ViT-giant, with embed-dim 1536 and 24 heads => embed-dim per head 64
    """
    model = DinoVisionTransformer(
