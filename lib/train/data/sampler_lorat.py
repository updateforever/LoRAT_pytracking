...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                template_frames, template_tensor_anno, meta_obj_train = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
                search_frames, search_tensor_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)

                template_tensor_frames = [self.img_numpy_to_tensor(f) for f in template_frames]
                search_tensor_frames = [self.img_numpy_to_tensor(f) for f in search_frames]
                template_numpy_anno = [b.numpy() for b in template_tensor_anno['bbox']]
                search_numpy_anno = [b.numpy() for b in search_tensor_anno['bbox']]
    ...
    def img_numpy_to_tensor(self, image: np.ndarray):
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            enough_visible_frames = visible.type(torch.int64).sum().item() > 2 * (
                    self.num_search_frames + self.num_template_frames) and len(visible) >= 4
