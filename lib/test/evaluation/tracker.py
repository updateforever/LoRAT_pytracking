...
import numpy as np
import torch
from soi.screening_util import find_local_maxima_v1, mask_image_with_boxes, iou4list, mask_image_with_boxes_online
import json
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            # seq.ground_truth_rect[frame_num]
    ...
    def extract_candidate_data(self, data, filte_mode='alpha', th=0.25):
        ...
        if len(score_map.shape) <= 4:
            side_len = int(np.sqrt(score_map.shape[-1]))  # 恢复为 27 × 27
            score_map = score_map.reshape(score_map.shape[:-1] + (side_len, side_len))
        
        all_boxes = torch.tensor(all_boxes, dtype=torch.float32)  # [729, 4]
        all_boxes = all_boxes.view(side_len, side_len, 4).permute(2, 0, 1).unsqueeze(0)
    ...
    def visualize_heatmap(self, seq_name, frame_candidate_data, frame_id):
        ...
        # 去除多余的维度
        frame_search_img = np.squeeze(frame_search_img)
        # 若是 (C, H, W)，转 (H, W, C)
        if frame_search_img.ndim == 3 and frame_search_img.shape[0] in [1,3]:
            frame_search_img = np.transpose(frame_search_img, (1, 2, 0))
        ...
        # 融合热力图与原图
        # 若 frame_search_img 是 float32 或别的dtype
